...
pub struct Lyrics {
    ...
    pub position: Signal<u64>,
}

impl Lyrics {
    pub fn new(signals: LyricsSignals, position: Signal<u64>) -> Self {
        let lines = memo(move |_| {
            if let Some(Some(content)) = signals.content.value() {
                parse_lrc(&content)
            } else {
                Vec::new()
            }
...
impl<'a> Widget for LyricsWidget<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let first_ts = self.lines.first().map(|(t, _)| *t).unwrap_or(0);
        ...
        let mut current_ts = self.lines.get(idx).map(|(t, _)| *t).unwrap_or(0);
        ...
        for &relative_pos in &positions_to_render {
            ...
            let (_, line_text) = &self.lines[line_idx];
            ...
            let base_color = if intro_wait_active {
                ...
                animate_line_color(line_idx, idx, eased_scroll, accent_color, muted_color)
            };

            let style = if intro_wait_active && relative_pos == 0 {
                Style::default()
                    .fg(blend_color_bg(base_color, opacity))
                ...
                Style::default().fg(blend_color_bg(base_color, opacity))
            };
            ...
            if !display_text.is_empty() || is_waiting {
                render_line(buf, inner, y_float, &display_text, style);
            }
        }
...
fn render_line(buf: &mut Buffer, area: Rect, y_float: f64, text: &str, style: Style) {
    ...
    } else if y_bottom >= area.y && y_bottom < area.y + area.height {
...
fn animate_line_color(
    ...
    if is_old_current {
        blend_colors(accent, muted, scroll_progress)
    } else if is_new_current {
        blend_colors(muted, accent, scroll_progress)
    } else {
        muted
    }
}

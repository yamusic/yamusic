...
pub const LOWPASS_MIN_HZ: f32 = 20.0;
pub const LOWPASS_MAX_HZ: f32 = 20_000.0;
pub const Q_MIN: f32 = 0.5;
pub const Q_MAX: f32 = 50.0;
pub const Q_DEFAULT: f32 = 1.0;

pub struct LowpassRenderer {
    meta: EffectMeta,
}

impl Default for LowpassRenderer {
    fn default() -> Self {
        Self::new()
    }
}

impl LowpassRenderer {
    pub fn new() -> Self {
        Self {
            meta: EffectMeta {
                id: "lowpass",
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect, vals: &[f32], accent: Color) {
        ...
        let cutoff = vals.first().copied().unwrap_or(5000.0);
        ...
        let cutoff_pos = map_to_width(cutoff, LOWPASS_MIN_HZ, LOWPASS_MAX_HZ, width);
        let q_norm = normalize_q(q, Q_MIN, Q_MAX);

        let resonance_headroom = 2.min(max_row);
        let passband_row = resonance_headroom;
        ...
        for x in 0..width {
            let row = if x < cutoff_pos {
                ...
                if resonance_gain > 0.05 && dist_from_cutoff < resonance_width {
                    ...
                    let peak_offset =
                        (resonance_gain * resonance_headroom as f32 * t).round() as usize;
                ...
                let stopband_width = if q > 50.0 {
                    ((width - cutoff_pos) as f32 * 0.15).max(3.0)
                } else {
                    ((width - cutoff_pos) as f32 * 0.6).max(5.0)
                };
        ...
        for x in 0..width {
            ...
            let (fill_top, fill_bottom) = if x == 0 {
                ...
                (prev_row.min(current_row), prev_row.max(current_row))
            };
            ...
            if is_cutoff {
                ...
                for r in fill_top..=fill_bottom.min(max_row) {
